...
surfacephase = inline('2*k*R*sqrt(1-(r./R).^2)','r','R','k');
referencephase = inline('2*k*R*(1-0.5*(r./R).^2)','r','R','k');

rtmp = linspace(-a,a);
Er = surfacephase(rtmp,R,k);
E0 = referencephase(rtmp,R,k);

figure, plot(rtmp,Er,rtmp,E0), grid
figure, plot(rtmp,Er-E0), grid

figure, plot(rplot,unwrap(angle(Er))/pi,rplot,unwrap(angle(E0))/pi), grid
figure, plot(rplot,angle(Er.*conj(E0))/pi), grid
