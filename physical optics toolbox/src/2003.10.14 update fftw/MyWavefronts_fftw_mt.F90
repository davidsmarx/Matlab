...
!  10/12/04
!
!  modified to use critical sections when calling fftw_plan_xxx
!  and DFFTW_DESTROY_PLAN subroutines because they are not
!  thread-safe. This module should now be thread safe for use
...
!   PUBLIC :: CONJG
   ...
   !! MY ADDITIONS:
   PUBLIC :: ReadWavefrontUNF
   PUBLIC :: WavefrontCumSum
   PUBLIC :: ScalarTimesWavefront
   PUBLIC :: CreateWavefront
   PUBLIC :: GetWavefrontCoords
   PUBLIC :: WavefrontCurvature
   PUBLIC :: ClipWindow
   PUBLIC :: ApplyArbitraryMask
   PUBLIC :: AssignFFTWPlanLock
   PUBLIC :: AssignReadWriteUNFLock
   ...
   TYPE, PUBLIC :: wavefront
      ...
      INTEGER*4                             :: lpFFTWPlanLock = 0 ! for use in critical sections, initialized in top level program
      INTEGER*4                             :: lpReadWriteUNFLock = 0 ! critical section lock when read or write to file
      INTEGER*4                             :: dummy ! for alignment
...
CONTAINS
  ...
  != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
   ...
   ! copy over multi-thread info, which is needed for critical section inside TestWavefrontAllocation
   beam%lpFFTWPlanLock = oldBeam%lpFFTWPlanLock
   beam%lpReadWriteUNFLock = oldBeam%lpReadWriteUNFLock
  ...
  != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
   ...
   ! if this call is within a multi-threaded process, need to lock out other
   ! threads because the fftw allocate calls are not thread-safe
   IF ( beam%lpFFTWPlanLock .NE. 0 ) THEN
        call EnterCriticalSection( beam%lpFFTWPlanLock )
   END IF

   ! deallocate old wavefront, if necessary
   ...
   ! unlock if necessary
   IF ( beam%lpFFTWPlanLock .NE. 0 ) THEN
        call LeaveCriticalSection( beam%lpFFTWPlanLock )
   END IF
  ...
  != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
   ...
   IF ( SIZE(refBeam%amp,1) /= Ny                             .OR.  &
      ...
      write(10,*) "Heterodyne:"
      write(10,*) "size(sigBeam%amp) = ", Nx, Ny
      write(10,*) "size(refBeam%amp) = ", size(refBeam%amp,2), size(refBeam%amp,1)
      write(10,*) "sigBeam%dx, dy = ", sigBeam%dx, sigBeam%dy
      write(10,*) "refBeam%dx, dy = ", refBeam%dx, refBeam%dy
  ...
  != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
   ...
   IF ( beam%lpFFTWPlanLock .NE. 0 ) THEN
        call EnterCriticalSection( beam%lpReadWriteUNFLock )
   END IF
   ...
   IF ( beam%lpFFTWPlanLock .NE. 0 ) THEN
        call LeaveCriticalSection( beam%lpReadWriteUNFLock )
   END IF
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! M Y   A D D I T I O N S !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
   SUBROUTINE ReadWavefrontUNF( beam, fileName, ioUnit )
  != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =

   ! Reads wavefront data to an unformatted file, for off-line analysis. The
   ! output file is given the name "fileName.unf" and it can be read by MATLAB.
   !
   !  USES NON-STANDARD FORTRAN 95
   !
   ...
   INTEGER, PARAMETER :: ok = 0
   INTEGER            :: j, k
   REAL(pr)           :: ramp, wavelength, curvature
   ! . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
   ...
   END IF
    ...
    j = INDEX( fileName, ".unf" )
    IF ( j==0 ) THEN
        outputName = TRIM( fileName )//".unf"
    ELSE
        outputName = TRIM( fileName )
    ENDIF
    

   unitNo = defaultUnit
   IF ( PRESENT( ioUnit ) ) unitNo = ioUnit

   OPEN( unitNo, FILE     = TRIM( outputName ),	&
						   STATUS   = "UNKNOWN",						&
						   FORM     = "BINARY",	  				&  ! NON-STANDARD
                           ACCESS   = "SEQUENTIAL",			&
						   ACTION   = "READ",					&
   ...
   IF ( ioFlag /= ok ) &
        CALL ErrorExit( "ReadWavefrontUNF: error opening input file" )

    !! read Nx and Ny first so that beam can be allocated
    READ( unitNo, IOSTAT = ioFlag ) Nx, Ny

    IF ( ioFlag == ok ) THEN

        !! allocate beam and read the data
        CALL TestWavefrontAllocation( beam, Nx, Ny )

        READ( unitNo ) beam%wavelength, beam%curvature

        READ( unitNo ) ( beam%x(k), k=1,Nx ),  ( beam%y(j), j=1,Ny )

!        beam%wavelength = wavelength
!        beam%curvature  = curvature
        beam%dx = (beam%x(Nx)-beam%x(1))/REAL(Nx-1, KIND=pr)
        beam%dy = (beam%y(Nx)-beam%y(1))/REAL(Ny-1, KIND=pr)

        !!! read real part first then imaginary
        DO k = 1, Nx
            Do j = 1, Ny
                READ( unitNo ) ramp  ! a real number
                beam%amp(j,k) = CMPLX(ramp, 0.0_pr, KIND=prc) ! convert real number to complex
            EndDo
        EndDo
        DO k = 1, Nx
            DO j = 1, Ny
                READ( unitNo ) ramp
                beam%amp(j,k) = beam%amp(j,k) + CMPLX(0.0_pr, ramp, KIND=prc)
            EndDo
        EndDo

    ELSE
      WRITE( ioUnit, "(I3)" ) unitNo
      CALL ErrorExit( "ReadWavefrontUNF: error reading from unit "//TRIM(outputName) )
    END IF
...
END SUBROUTINE ReadWavefrontUNF


!= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
   SUBROUTINE WavefrontCumSum( beam1, beam2 )
  != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =

   ! Adds the complex values of two input wavefronts and replaces beam1 with
   ! the result. The originial beam1 data is lost
   ! D. Marx, Aug. 12, 2003
   !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   TYPE(wavefront), INTENT(INOUT)  :: beam1
   TYPE(wavefront), INTENT(IN)  :: beam2
   ...
   ELSE
      ...
      ! Usum = exp(i k R2) * (u1 exp(i k (R1-R2)) + u2)
      ! Apply curvature difference to beam1 before summing amplitudes
      piston = 0.0_pr
      tilt   = 0.0_pr
      curvCoeff = PI*(beam1%curvature - beam2%curvature)/beam2%wavelength
      CALL QuadraticPhase( beam1%amp, beam1%x, beam1%y, &
      							piston, tilt, tilt, curvCoeff )
      beam1%curvature = beam2%curvature
      
      ! now sum
      DO k = 1,Nx
      	DO j = 1,Ny
      		beam1%amp(j,k) = beam1%amp(j,k) + beam2%amp(j,k)
      	END DO
      END DO
   END IF

   END SUBROUTINE WavefrontCumSum

   !=======================================================================
   SUBROUTINE ScalarTimesWavefront( coeff, beam )
   !======================================================================---
   ! multiply a complex number times the wavefront amplitude
   ! D. Marx 3 July 03

   IMPLICIT NONE

   TYPE(wavefront), INTENT(INOUT) :: beam
   COMPLEX(prc),    INTENT(IN)    :: coeff
   ...
   DO k = 1,Nx
      DO j = 1,Ny
         beam%amp(j,k) = coeff * beam%amp(j,k)
      END DO
   END DO

   END SUBROUTINE ScalarTimesWavefront
  
!= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
   SUBROUTINE CreateWavefront( beam, field, dx, dy, lambda, curv,  &
                             xc, yc, power )
  != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =

   ! Create a wavefront object with values according to the input arguments
   ! Initial curvature is zero (i.e., flat wavefront)
   !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   COMPLEX(prc), DIMENSION(:,:), INTENT(IN) :: field  ! the complex field
   REAL(pr), INTENT(IN)           :: dx, dy           ! Sample spacing
   REAL(pr), INTENT(IN)           :: lambda           ! Beam wavelength
   REAL(pr), INTENT(IN), OPTIONAL :: curv             ! curvature
   REAL(pr), INTENT(IN), OPTIONAL :: xc, yc           ! Beam center
   REAL(pr), INTENT(IN), OPTIONAL :: power            ! Beam power
   ...
   ! get the size of the wavefront from the field
   ny = SIZE(field,1)
   nx = SIZE(field,2)
   ...
   beam%dx = dx
   beam%dy = dy
   beam%x = CoordinateVector( Nx, dx, xc )
   beam%y = CoordinateVector( Ny, dy, yc )

   beam%amp = field

   beam%wavelength = lambda
   ...
   IF ( PRESENT( power ) ) CALL NormalizeToPower( beam, power )

   END SUBROUTINE CreateWavefront


!==========================================================================================
    SUBROUTINE GetWavefrontCoords(beam, x, y)
    ! input: beam = a wavefront
    ! output: x = an x coordinate vector

    IMPLICIT NONE

    Type(wavefront), INTENT(IN)  :: beam
    REAL(pr), INTENT(INOUT), OPTIONAL, ALLOCATABLE, DIMENSION(:) :: x
    REAL(pr), INTENT(INOUT), OPTIONAL, ALLOCATABLE, DIMENSION(:) :: y
    ...
    END SUBROUTINE GetWavefrontCoords


 != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
   FUNCTION WavefrontCurvature( beam ) RESULT( curv )
  != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =

   ! Returns value of curvature for the input wavefront
   ...
   END FUNCTION WavefrontCurvature


!= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
   SUBROUTINE ClipWindow( beam, winT, winA, xc, yc, rotAngle, obsc )
   
!   WindowClip(wavefront, winT, winA, xc, yc, rotAngle, obsc)
  != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =

   ! Calls for clipping -- aperture or obscuration where the shape is determined
   ! by a weighting window, such as the Blackman window. xc and yc are the offset
   ! of the aperture, ant rotAngle is the counterclockwise rotation of the aperture.
   ! The aperture is rotated before it is translated.
   !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   REAL(pr),        INTENT(IN), OPTIONAL :: xc, yc
   LOGICAL,         INTENT(IN), OPTIONAL :: obsc
   REAL(pr),        INTENT(IN), OPTIONAL :: rotAngle

   INTRINSIC EPSILON, PRESENT, ABS, COS

   ! Local variables
   REAL(pr)  :: rottmp  = 0.0_pr
   REAL(pr)  :: xctmp   = 0.0_pr
   REAL(pr)  :: yctmp   = 0.0_pr
   LOGICAL   :: obsctmp = .FALSE.

   !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

   IF ( winT < EPSILON( winT ) ) RETURN

   IF ( PRESENT( rotAngle ) ) rottmp = rotAngle

   IF ( PRESENT( xc ) ) xctmp = xc

   IF ( PRESENT( yc ) ) yctmp = yc

   IF ( PRESENT( obsc ) ) obsctmp = obsc

   CALL WindowClip( beam%amp, beam%x, beam%y, beam%dx, beam%dy, winT, winA, &
            xctmp, yctmp, rottmp, obsctmp)


   END SUBROUTINE ClipWindow


!===========================================================================
SUBROUTINE ApplyArbitraryMask( beam, mask, negative )

!! apply an arbitrary mask as described in the wavefront mask to the wavefront beam
!! if negative = .FALSE. (default), beam = mask * beam
!! if negative = .TRUE.             beam = (1-mask) * beam

    IMPLICIT NONE

    TYPE(wavefront), INTENT(INOUT)        :: beam
    TYPE(wavefront), INTENT(IN)           :: mask
    ...
    ! default values
    negtmp = .FALSE.
    IF ( PRESENT( negative ) ) negtmp = negative

    ! check that beam and mask are the same size
    nx = SIZE(beam%x)
    ny = SIZE(beam%y)
    IF ( nx .NE. SIZE(mask%x) ) call ErrorExit( "ApplyArbitraryMask: beam and mask unequal Nx" )
    IF ( ny .NE. SIZE(mask%y) ) call ErrorExit( "ApplyArbitraryMask: beam and mask unequal Ny" )
    IF ( beam%dx .NE. (mask%dx) ) call ErrorExit( "ApplyArbitraryMask: unequal dx" )
    IF ( beam%dy .NE. (mask%dy) ) call ErrorExit( "ApplyArbitraryMask: unequal dy" )

    ! apply the mask, depending on value of negative
    IF ( negtmp ) THEN
        DO ix = 1, Nx
            DO iy = 1, Ny
       	    	beam%amp(iy,ix) = (1.0_pr - mask%amp(iy,ix)) * beam%amp(iy,ix)
           	END DO
        END DO
    ELSE
        DO ix = 1, Nx
            DO iy = 1, Ny
                beam%amp(iy,ix) = mask%amp(iy,ix) * beam%amp(iy,ix)
            END DO
        END DO
    ENDIF

END SUBROUTINE ApplyArbitraryMask


!====================================================================================
SUBROUTINE AssignFFTWPlanLock( beam, lpFFTWPlanLock )
!
! Put the declaration:
! type(RTL_CRITICAL_SECTION) :: FFTWPlanLock
! is in the calling routine. Use this subroutine
! to then assign the lock
! to each wavefront that may be used in parallel in
! multiple threads with the call:
! AssignFFTWPlanLock( beam, loc(FFTWPlanLock) )
! Any calls to TestWavefrontAllocation during the execution
! of the multiple threads will then block other threads from
! calling the fftw_plan_xxx for wavefronts that have the same
! lock.

    IMPLICIT NONE

    TYPE(wavefront), INTENT(INOUT)        :: beam
    integer*4,       INTENT(IN)           :: lpFFTWPlanLock 

    beam%lpFFTWPlanLock = lpFFTWPlanLock


END SUBROUTINE AssignFFTWPlanLock

!====================================================================================
SUBROUTINE AssignReadWriteUNFLock( beam, lpReadWriteUNFLock )
!

    IMPLICIT NONE

    TYPE(wavefront), INTENT(INOUT)        :: beam
    integer*4,       INTENT(IN)           :: lpReadWriteUNFLock 

    beam%lpReadWriteUNFLock = lpReadWriteUNFLock


END SUBROUTINE AssignReadWriteUNFLock
