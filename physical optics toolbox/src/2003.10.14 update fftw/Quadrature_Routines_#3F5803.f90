!     Last change:  RSB  10 Dec 2004    4:21 pm
...
CONTAINS
  ...
  != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
   ...
   CONTAINS
     ...
     !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      !  10 Dec  '04    Corrected calculation of coefficients
      !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      coeffs(3) = ( f(3) - 2.0_pr*f(2) + f(1) )/(2.0_pr*dz**2)
      coeffs(2) = ( f(3) - f(1) )/(2.0_pr*dz) -            &
                  0.5_pr*coeffs(3)*( z(3)+2.0_pr*z(2)+z(1) )
      coeffs(1) = 0.0_pr
      DO k = 1,3
         coeffs(1) = coeffs(1) + ( f(k) - z(k)*( coeffs(2) + z(k)*coeffs(3) ) )
      END DO
      coeffs(1) = coeffs(1)/3.0_pr
  ...
  != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
   ...
   CONTAINS
     ...
     !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      IF ( N < 7 ) CALL ErrorExit( "OneDquadCmplx: too few sample points" )
  ...
  != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
   ...
   ! of the integration interval, which in general do not coincide with function
   ! sample points. The input vector contains function values at uniformly-spaced
   ! intervals of the independent variable, x.
   !   The matrix Mfit results from a cubic fit to four points that straddle the
   ! integration endpoints. It is used to calculate the contributions from
   ! the partial sample intervals near xLower and xUpper when these integration
   ! limits do not coincide with function sample points.
   ...
   !      9 Dec  '04    Corrected calculation of end-point contributions
   !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   ! Find sample points k1 and k2 such that
   ! x(k1-1) <= xLower < x(k1)   and    x(k2) < xUpper <= x(k2+1).
   ! Ordinarily, k1 <= k2; but it is possible that k2 = k1-1.
   ...
   coeff1 = MATMUL( Mfit(:,4:1:-1), vector(k1-2:k1+1) )
   coeff2 = MATMUL( MFIT, vector(k2-1:k2+2) )

   ! Integral from x(k1-1) to x(k1)
   firstInterval = dx * SUM( coeff1 )

   ! Contributions from the ends of the integration interval
   ! (fractions of the sample interval)
   lowerEnd = (x(k1)-xL)*EvalPoly( (x(k1)-xL)/dx, coeff1 )
   upperEnd = (xU-x(k2))*EvalPoly( (xU-x(k2))/dx, coeff2 )

   ! If k2 = k1, this is all there is...
   ...
   ELSE IF ( k2 == k1+1 ) THEN
      ...
      ! Do a Simpson quadrature using 3 function sample points,
      ! then subtract the contribution from the first interval.
      integral = integral - firstInterval +                                   &
                 dx * ( vector(k1-1) + 2.0_pr*vector(k1) + vector(k2) )/3.0_pr

   ELSE IF ( k2 > k1+1 ) THEN

      ! Use Simpson quadrature for interior points (kStart to k2)
      kStart = k1
      IF ( MODULO( k2-k1, 2 ) /= 0 ) THEN
         ! Do quadrature over one extra interval (to have an even number
         ! of sample intervals), then subtract the  contribution from the
         ! extra (first) sample interval.
         kStart   = k1-1
         integral = integral - firstInterval
      END IF
  ...
  != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
   ...
   IF ( k1 < 3 ) THEN
        write(10,*) "nx = ", Nx
        write(10,*) "dx = ", dx
        write(10,*) "xLower = ", xLower
        write(10,*) "xUpper = ", xUpper
        write(10,*) "k1 = ", k1, " k2 = ", k2
        CALL ErrorExit( "Quad1DCmplx: not enough samples at lower end" )
   ENDIF
