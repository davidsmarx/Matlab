!     Last change:  RSB  15 Nov 2000    3:18 pm
MODULE Singleton_FFT
...
!============================
  ...
  !
  !
  ! Scaling:   !!! NOT DONE IN THIS VERSION !!!
  !
  ...
  PRIVATE
  PUBLIC:: fft, fftn, fftkind, kindcpx

  INTEGER, PARAMETER:: fftkind = pr
  INTEGER, PARAMETER:: kindcpx = prc
...
CONTAINS
 ...
 != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
    ...
    !--- formal parameters
    COMPLEX(kindcpx), DIMENSION(:), INTENT(IN)           :: array
    INTEGER,          DIMENSION(:), INTENT(IN),  OPTIONAL:: dim
    LOGICAL,                        INTENT(IN),  OPTIONAL:: inv
    INTEGER,                        INTENT(OUT), OPTIONAL:: stat
    !--- function result
    COMPLEX(kindcpx), DIMENSION(SIZE(array, 1)):: ft
    !--- intrinsics used
 ...
 != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
    ...
    INTEGER,                          INTENT(OUT), OPTIONAL:: stat
    !--- function result
    COMPLEX(kindcpx), DIMENSION(SIZE(array, 1), SIZE(array, 2)):: ft
    !--- intrinsics used
    INTRINSIC SIZE, SHAPE
 ...
 != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
    ...
    !--- formal parameters
    COMPLEX(kindcpx), DIMENSION(*), INTENT(INOUT)        :: array
    INTEGER,          DIMENSION(:), INTENT(IN)           :: shape
    INTEGER,          DIMENSION(:), INTENT(IN),  OPTIONAL:: dim
    LOGICAL,                        INTENT(IN),  OPTIONAL:: inv
    INTEGER,                        INTENT(OUT), OPTIONAL:: stat
...
!!  REAL(fftkind):: scale
    ...
    ! SCALING IS NOT DONE IN THIS VERSION OF THE ROUTINE
 ...
 != = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
    ...
    COMPLEX(kindcpx), DIMENSION(:), ALLOCATABLE  :: ctmp
    ...
    COMPLEX(kindcpx):: cc, cj, ck, cjp, cjm, ckp, ckm
    !--- intrinsics used
               ...
               DO
                  DO
                     DO
                        ...
                        array(k2) = ck * CMPLX(c1, s1, kind=kindcpx)
                        kk = k2 + kspan
                        IF (kk >= nt) EXIT
                     END DO
                     k2 = kk - nt
            ...
            DO
               ...
               DO
                  DO
                     ...
                     cjp = ckp - cjp
                     IF (inv) THEN
                        ckp = ckm + CMPLX(-AIMAG(cjm), REAL(cjm), kind=kindcpx)
                        ckm = ckm + CMPLX(AIMAG(cjm), -REAL(cjm), kind=kindcpx)
                     ELSE
                        ckp = ckm + CMPLX(AIMAG(cjm), -REAL(cjm), kind=kindcpx)
                        ckm = ckm + CMPLX(-AIMAG(cjm), REAL(cjm), kind=kindcpx)
                     END IF
                     ...
                     IF (s1 == 0.0_fftkind) THEN
                        ...
                        array(k3) = ckm
                     ELSE
                        array(k1) = ckp * CMPLX(c1, s1, kind=kindcpx)
                        array(k2) = cjp * CMPLX(c2, s2, kind=kindcpx)
                        array(k3) = ckm * CMPLX(c3, s3, kind=kindcpx)
                     END IF
                     kk = k3 + kspan
                     IF (kk > nt) EXIT
                  END DO
         ...
         CASE default
            ...
            CASE (3) !-- transform for factor of 3 (optional code)
               DO
                  DO
                     ...
                     array(k1) = ck + CMPLX(-AIMAG(cj), REAL(cj), kind=kindcpx)
                     array(k2) = ck + CMPLX(AIMAG(cj), -REAL(cj), kind=kindcpx)
                     kk = k2 + kspan
                     IF (kk >= nn) EXIT
                  END DO
                  kk = kk - nn
            ...
            CASE (5) !-- transform for factor of 5 (optional code)
               ...
               DO
                  DO
                     ...
                     cj = ckm * s72 + cjm * s2
                     array(k1) = ck + CMPLX(-AIMAG(cj), REAL(cj), kind=kindcpx)
                     array(k4) = ck + CMPLX(AIMAG(cj), -REAL(cj), kind=kindcpx)
                     ck = ckp * c2 + cjp * c72 + cc
                     cj = ckm * s2 - cjm * s72
                     array(k2) = ck + CMPLX(-AIMAG(cj), REAL(cj), kind=kindcpx)
                     array(k3) = ck + CMPLX(AIMAG(cj), -REAL(cj), kind=kindcpx)
                     kk = k4 + kspan
                     IF (kk >= nn) EXIT
                  END DO
                  kk = kk - nn
               ...
               DO
                  DO
                     ...
                     DO
                        ...
                        jj = j
                        ck = cc
                        cj = CMPLX(0.0_fftkind, 0.0_fftkind, kind=kindcpx)
                        k = 1
                        DO
                           k = k + 1
            ...
            DO
               ...
               DO
                  ...
                  DO
                     DO
                        array(kk) = CMPLX(c2, s2, kind=kindcpx) * array(kk)
                        kk = kk + ispan
                        IF (kk > nt) EXIT
                     END DO
                     ak = s1 * s2
