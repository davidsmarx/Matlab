function winpath = PathTranslator(s383path)
% winpath = PathTranslator(s383path)

if ~ispc, %isequal(computer, 'GLNXA64'),
    winpath = s383path;
    return
end

% init output
winpath = '';

% special case: if fullfile() was call first, all the '/' are '\', check
% for this:
if isequal(s383path(1), '\')
    s383path = replace(s383path, '\', '/');
end

A = regexp(s383path,'/','split');

if length(A) <= 2,
    winpath = s383path;
    return
end

if strcmp(A{1}, '~'),
    winpath = fullfile('Y:\', A{2:end});
    
elseif ~strcmp(s383path(1), '/')
    winpath = s383path; % it's a relative path
        
elseif strcmp([A{2} '/' A{3}], 'home/dmarx'),
    winpath = strjoin({'Y:', A{4:end}}, filesep);
    
elseif isequal([A{2} '/' A{3}],'proj/hcit2')
    % \\s383-nfs = /proj
    %winpath = '\\s383-nfs\hcit2'; %
    winpath = fullfile('Y:\', A{5:end}); % A{4} = dmarx
    
elseif length(A) >= 6 && isequal(fullfile(A{2:6}), 'proj\hcit\home\dmarx\HCIT')
    % \\s383-nfs = /proj
    %winpath = '\\s383-nfs\hcit'; %
    winpath = fullfile('Y:\links', A{6:end});
    
elseif isequal([A{2} '/' A{3}],'proj/afta-im')
    % \\s383-nfs = /proj
    winpath = '\\s383-nfs\afta-im'; %

elseif length(A) >= 5 && isequal([A{2} '/' A{3} '/' A{4}],'proj/hcit/data')
    winpath = strjoin({'Y:\links\ln_hcit_data', A{5:end}}, filesep);

elseif isequal([A{2} '/' A{3} '/' A{4}], 'proj/mcb/data')
    % \\s383-nfs = /proj
    winpath = fullfile('Y:\links\ln_mcb_data', A{5:end});

elseif isequal([A{2} '/' A{3} '/' A{4}],'home/hcit/OMC')
    winpath = fullfile('Y:\links\ln_hcit_omc', A{5:end});
    
elseif isequal([A{2} '/' A{3}], 'proj/lowfs')
    winpath = fullfile('Y:\links\ln_lowfs', A{4:end});
    
elseif isequal([A{2} '/' A{3}],'net/spud-data')
    % mcb camera data
    winpath = 'Y:\HCIT\ln_spud_data_Data';
    A = {A{1:3},A{5:end}}; % remove /Data/

elseif isequal([A{2} '/' A{3}],'net/piaa-data') || isequal([A{2} '/' A{3}],'proj/piaa-data')
    % mcb camera data
    winpath = fullfile('X:', A{5:end});

elseif isequal([A{2} '/' A{3}], 'proj/piaacmc')
    % piaacmc share and camera images
    winpath = fullfile('W:', A{4:end});
    
elseif length(A) >= 4 && isequal([A{2} '/' A{3} '/' A{4}],'proj/dst/data')
    winpath = fullfile('Y:\links\ln_dst_data', A{5:end});
    
elseif isequal(A{1}, '..') || isequal(A{1}, '.'),
    % relative path, no need to translate
    winpath = s383path;
    
else,
    warning(['no translation for ' s383path]);
    winpath = s383path;
    return
end

% for ii = 4:length(A),
%     winpath = [winpath '\' A{ii}];
% end
